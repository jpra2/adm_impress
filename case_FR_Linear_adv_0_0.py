...
for arq in arquivos:
    if  arq.startswith(name):
        ...
        plt.xlabel('Dimensionless distance')
        plt.title('Water horizontal displacement')
        plt.savefig('results/compositional/FR_paper/sinoidal_pulse/Nk_seno_64_func.eps', format='eps')
        ...
        y_ref = -2*x +2
        plt.plot(x, y, 'r', x, y_FR3, 'g', x, y_FR4, 'y', x[:-1], y_FR5, 'b')
        plt.grid()
        plt.ylabel('$log_{10}({E}_{L_1})$')
        plt.xlabel('$log_{10}(N)$')
        plt.legend(('FR-2nd order', 'FR-3rd order', 'FR-4th order', 'FR-5th order'))
        plt.savefig('results/compositional/FR_paper/sinoidal_pulse/Nk_seno_convergence.eps', format='eps')
        import pdb; pdb.set_trace()
