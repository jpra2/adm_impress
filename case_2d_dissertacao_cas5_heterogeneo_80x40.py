...
x_ref = np.array([0, 26.8578, 31.3409, 34.0468, 38.9338, 50])
y_ref = np.array([1, 0.95, 0.65, 0.35, 0.05, 0])

for arq in arquivos:
    if  arq.startswith(name):

        datas = np.load('flying/Heterogeneo_plot/results_Hoteit_Firoo_2k_ex5a_80x40_FI_VPI0_43_PLOT_1267.npy', allow_pickle=True)
        ...
        plt.legend(('Solução de referência', 'FI 80x40 CVs'))
        plt.ylabel('Fração molar global do metano em y=2,50m')
        plt.xlabel('Distância (m)')
        plt.grid()
        plt.savefig('results/caso5_80x40_J2_' + '.png')
