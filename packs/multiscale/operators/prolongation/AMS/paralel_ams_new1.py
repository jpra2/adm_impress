...
class SubDomain(CommonInfos):
	...
	def __init__(self,
		...
		if len(total_source_term) == 0:
			self.l_total_source_term = total_source_term
		else:
		...
		shape_T_local = self.l_T.shape[0]
		self.g_local_ids = g_local_ids
		n_volumes = len(g_local_ids)
		assert n_volumes == shape_T_local
...
class LocalOperator:
	...
	def __init__(self,
		...
		self.comm = comm
		self.locker = locker
		self.proc = LocalOperator.all_proc
		LocalOperator.all_proc += 1
	...
	def run(self):
		...
		for subd in self.subdomains:
			operator = AMSTpfa(subd.local_ids, subd.l_primal_ids, subd.l_dual_flags, proc=self.proc, locker=self.locker)

			op, pcorr = operator.run(subd.l_T, total_source_term=subd.l_total_source_term)
...
class MasterOP:
	def __init__(self,
		...
		list_of_subdomains = self.get_list_subdomains(T, all_dual_subsets, total_source_term=total_source_term)
	...
	def get_list_subdomains(self, T, all_dual_subsets, total_source_term):
		list_of_subdomains = []
		for dual_subset in all_dual_subsets:
			...
			list_of_subdomains.append(SubDomain(T, local_gids, dual_ids, primal_ids, total_source_term=total_source_term))
	...
	def run(self):
		...
		lock = mp.Lock()
		procs = [mp.Process(target=run_thing, args=[LocalOperator(obj, comm, lock)]) for obj, comm in zip(self.list_of_process_per_cpu, w2m)]
