...
class global_assembly:
    ...
    def get_momentum_matrix(self, fd ,M,dx,dy, dz,col):
        ...
        faces_viz_adjs_d=M.volumes.bridge_adjacencies(adjs_d.flatten(),2,2).reshape(len(fd),12)
        ...
        #fd_l=M.id_fint[fd]#-len(horizontal)
        ...
        for i in range(len(fd)):
            try:
                ...
                if len(laterais[i])==1:
                    M_d[fd_l[i],self.nv+fd_l_orig[i]]-=2*self.mi_l/(dx*dx)
