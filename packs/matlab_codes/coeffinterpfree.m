function [coefficient]=coeffinterpfree(kmap,F)
...
for ifacont=1:size(bedge,1)
    ...
    for jj=auxvetor
        if jj>size(bedge,1)
           c=inedge(jj-size(bedge,1),3:4)==lef;
           if c(1,1)==0
               r(ii,1)=inedge(jj-size(bedge,1),4);
           else
              r(ii,1)=inedge(jj-size(bedge,1),3); 
           end
           ii=ii+1;
        else
          r(ii,1)=lef;
...
for iface=1:size(inedge,1)
    ...
    auxvetor=F(rel,1:krel);
    ii=1;
    for jj=auxvetor
        if jj>size(bedge,1)
           c=inedge(jj-size(bedge,1),3:4)==rel;
           if c(1,1)==0
               s(ii,1)=inedge(jj-size(bedge,1),4);
           else
              s(ii,1)=inedge(jj-size(bedge,1),3); 
           end
           ii=ii+1;
        else
          s(ii,1)=lef;
