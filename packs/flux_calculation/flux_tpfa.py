...
class TpfaFlux:
    ...
    def get_gravity_source_term_old0(self):
    ...
    def get_gravity_source_term(self):
        '''
            gravity source term for new upwind method
        '''
        ...
        if self.gravity:
            ...
            # data = np.array([grav_source_term_water_internal_faces, -grav_source_term_water_internal_faces]).flatten()
            # self._data['grav_source_term_water_volumes'] = sp.csc_matrix((data, (lines, cols)), shape=(self.n_volumes, 1)).toarray().flatten()
            self._data['grav_source_term_water_volumes'] = self.g_w_volumes
            self._data['grav_source_term_water_faces'][internal_faces] = self.g_w_internal_faces

        self.data_impress[self.data_impress.variables_impress['flux_grav_volumes']] = self.g_total_volumes
        self.data_impress[self.data_impress.variables_impress['flux_grav_faces']][internal_faces] = self.g_total_internal_faces
    ...
    def get_flux_faces_and_volumes_old0(self) -> None:
    ...
    def get_flux_faces_and_volumes(self) -> None:
        ...
        # data = np.array([flux_internal_faces, -flux_internal_faces]).flatten()
        # flux_volumes = sp.csc_matrix((data, (lines, cols)), shape=(self.n_volumes, 1)).toarray().flatten()
        flux_sigma_internal_faces = self.flux_sigma_internal_faces
        velocity_faces = np.zeros(len(self.elements_lv0['faces']))
        velocity_faces[internal_faces] = flux_sigma_internal_faces/area_internal_faces
        velocity_faces = velocity_faces.reshape([len(self.elements_lv0['faces']), 1]) * u_normal

        self.data_impress['flux_volumes'] = self.flux_sigma_volumes
        self.data_impress['flux_faces'][internal_faces] = flux_sigma_internal_faces
