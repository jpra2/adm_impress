...
class StabilityCheck:
    ...
    def run_init(self, P, z, pflash = True, ponteiro_flash = []):
        ...
        ponteiro_flash[np.sum(self.z==1, dtype=bool)] = True
        #self.z[self.z<=0] = 1e-30 #resolver esse Bo de outro jeito
        #ponteiro_nc1 = np.zeros_like(self.z, dtype=bool)
        #ponteiro_nc1[self.z==1] = True
        #ponteiro_nc1 = ponteiro_nc1.sum(axis=0, dtype=bool)
        #if any(ponteiro_nc1): pflash = False
        #ponteiro_flash[ponteiro_nc1] = True
        ...
        #ponteiro_flash[ponteiro_nc1] = False

        if (any(ponteiro_flash) or pflash) and ctes.Nc>1:
    ...
    def check_phase_nc_1(self, ponteiro_nc1):
        Pv = self.vapor_pressure_pure_substancies()
        Pv = Pv*np.ones_like(self.z)
        self.x[:,ponteiro_nc1] = self.z[:,ponteiro_nc1]
        self.y[:,ponteiro_nc1] = self.z[:,ponteiro_nc1]
        L_aux = self.L[ponteiro_nc1]
        L_aux[self.P[ponteiro_nc1] > Pv[self.z==1]] = 1
        L_aux[self.P[ponteiro_nc1] < Pv[self.z==1]] = 0.
        self.L[ponteiro_nc1] = L_aux
        self.V = 1. - self.L
        self.K[:,ponteiro_nc1] = self.y[:,ponteiro_nc1]/self.x[:,ponteiro_nc1]
        self.K[self.x==0] = 1
