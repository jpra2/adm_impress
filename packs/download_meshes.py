...
from packs.defpaths import BASE_DIR, MESH_DIR, CACHE_DIR, MANIFEST_FILE
...
def ensure_mesh(
    ...
    if mesh_name not in meshes:
        ...
        raise ValueError(
            f"Malha '{mesh_name}' não está "
            f"no manifest.\n\n"
            f"Malhas disponíveis:\n"
            f"{available}"
        )

    mesh_info = meshes[mesh_name]
    ...
    if mesh_path.exists():
        ...
        print()
        print(
            f"A malha {mesh_name} já existe localmente."
        )
