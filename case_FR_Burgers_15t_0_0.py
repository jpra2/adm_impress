...
with open('Sw_Burguer_ref_sol.txt', 'w') as f:
    for item in Nk_ans:
        f.write("%s\n" % item)

with open('x_Burguer_re_sol.txt', 'w') as f:
    for item in x:
        f.write("%s\n" % item)
import pdb; pdb.set_trace()
for arq in arquivos:
    if  arq.startswith(name):
        ...
        plt.title('Results for 80 control volumes mesh')
        plt.savefig('results/compositional/FR_paper/Burger/Nk_Burgers_15t_80CV_4.eps', format='eps')
        ...
        plt.legend(('4th order', '3rd order', '2nd order', 'Reference solution'))
        plt.title('Results for 80 control volumes mesh')
        plt.savefig('results/compositional/FR_paper/Burger/Nk_Burgers_comparison_15t_80CV.eps', format='eps')
