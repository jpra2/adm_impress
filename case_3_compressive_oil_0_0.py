...
for arq in arquivos:
    if  arq.startswith(name):
        ...
        datas = np.load('flying/results_compressive_oil_case_FR_37.npy', allow_pickle=True)
        b = 0
        ...
        fig2 = plt.figure(2)
        plt.grid(fig2)
        plt.plot(x, pressure[0], 'bo')
        plt.plot(x_ans, p_ans[0], 'r')
        ...
        plt.legend(('FR - 2nd order', 'Solução Analítica'))
        plt.title('t = 5 dias')
        plt.savefig('results/compositional/FR/pressure_comp_oil_5days_FR2' + '.png')

        datas = np.load('flying/results_compressive_oil_case_32_FR4_36.npy', allow_pickle=True)
        for data in datas[1:]:
            pressure = data[4]/1e6
            Ft = data[13]
            x32 = np.linspace(0+1/32, 1-1/32, 31)

        datas = np.load('flying/results_compressive_oil_case_upw_148.npy', allow_pickle=True)
        for data in datas[1:]:
            pressure = data[4]/1e6
            Ft_upw = data[13]
            x = np.linspace(0+1/100, 1-1/100, 99)


        fig2 = plt.figure(3)
        plt.grid(fig2)
        plt.plot(x32, Ft[0], 'ro', mfc= 'none')
        plt.plot(x, Ft_upw[0], 'k')
        plt.legend(('FR 4th order - 32X1X1', 'Upwind 100x1x1'))
        plt.ylabel('Total Flux (MPa)')
        plt.xlabel('Distância Adimensional')
        plt.title('32x1x1 mesh - t = 5 dias')
        plt.savefig('results/compositional/FR/Ft_comp_oil_5days_FR4_32' + '.png')
