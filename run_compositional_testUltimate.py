from packs.directories import data_loaded, simulation_type
...
from packs.contours.wells_compositional import WellsCompositional
...
class run_simulation:
    ...
    def initialize(self, load, convert, mesh):
        ...
        M, elements_lv0, data_impress, wells = self.run(M, wells, fprop, load, mesh, convert)
    ...
    def run(self, M, wells, fprop, load, mesh, convert):
        ''' Function created to compute reservoir and fluid properties at each \
        time step '''
        M, elements_lv0, data_impress, wells = \
                                  initial_mesh(mesh, load=load, convert=convert)
        #import pdb; pdb.set_trace()
        t0 = time.time()
